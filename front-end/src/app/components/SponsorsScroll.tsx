...
export function SponsorScroll({ sponsors, direction = "left", speed = 30 }: SponsorScrollProps) {
  const [items, setItems] = useState<string[]>([])

  useEffect(() => {
    setItems([...sponsors, ...sponsors])
  }, [sponsors])
        ...
        {items.map((sponsor, idx) => (
          ...
          >
            <Image
              ...
              width={150}
              height={75}
              className="w-auto h-auto max-h-full max-w-[80%] object-contain filter grayscale transition-all duration-300"
            />
          </div>
        ))}
      </div>
